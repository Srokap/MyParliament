{*
  {if $dataset->fields}
	...
	<div class="s_browser_order">
	  ...
	  <input class="mBtn green s_browser_mbtn" type="submit" value="Sortuj" />
  </div>
  {/if}
*}
  ...
  {if $filters || $dataset->data.fts_columns}
	<div class="s_browser_filtry_ul_cont">
	  ...
	  <div class="filter_div{if !$f.parent_field} visible{/if}{if $f.selected} expanded{/if}" field="{$f.field}">
		  <h2><i></i> {$f.tytul}<span class="c">:</span></h2>
			<ul class="s_browser_filtry_ul"{if !$f.selected} style="display: none;"{/if}>
