var MBrowser = Class.create({
  ...
  initialize: function(){
    ...
    if( !this.params['add_new_enabled'] ) this.params['add_new_enabled']=true;
        
    // build
    with( this ) {
      
      var html = '<div class="mbrowser_header"><div class="topbar"><h2 class="title">'+title;
      if( params['add_new_enabled'] )
        html += ' <a class="add_new_a" href="#" onclick="return false;"><img src="/g/Add-icon.png" /></a>';
      html += '</h2></div><div class="categories"></div><div class="pagination">&nbsp;</div></div><div class="list _height_controll"></div><div class="item"></div></div>';
      div.update( html );
      
      this.add_new_a = div.down('.add_new_a').observe('click', this.add_new_a_click.bind(this));
  ...
  onLoad: function(data){
    ...
    if( this.controll_data[2]==0 ) {
      this.paginationDiv.update('<p class="pages">&nbsp;</p><p class="pages_info">Brak elementów</p>');
    } else {
  ...
  loadNextItem: function(){
    if( this.controll_data[2]>0 ) {
	    var selectedDiv = this.getSelectedItemDiv();
	    if( selectedDiv ) {
	      if( selectedDiv.next('.listitem') ) { this.loadItem( selectedDiv.next('.listitem').readAttribute('mid') ); } else {
	        this.closeItem();
	        this.load(0);
	      }
	    } else {
  ...
  addItemButton: function(className, label, onClick){
    if( this.itemButtonsDiv ) {
      var btn = new Element('input', {type: 'button', className: 'btn '+className, value: label});
  ...
  toggleFullscreen: function(){
    ...
    $M.heightController.global_fix = this._fullscreen ? 43 : 0;
    $$('body').first().toggleClassName('mbrowser_fullscreen');
    $M.heightController.resizeArea('_COL_RIGHT');
    if( Object.isFunction(this.onToggleFullscreen) ) this.onToggleFullscreen( this._fullscreen );
  },
  ...
  onLoadItem: function(html){
    ...
    if( this.params['standard_title_bar'] ) {
      ...
      this.addItemButton('close', 'O', this.toggleFullscreen.bind(this));  
    }
