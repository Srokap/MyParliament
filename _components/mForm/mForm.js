...
var mFormTypes = Class.create({
  ...
  register: function(type, className){
    this.types.push([type, className]);
  },
...
var mFormField = Class.create({
  initialize: function(params){
    ...
    if( params.value && params.value!=='0' ) { this.setValue( params.value ); } else {
...
_mFormTypes.register('select', Class.create(mFormField, {
  getValueDiv: function(){
    ...
    for( var i=0; i<this.options.length; i++ ) {
      var option = new Element('option', {value: this.options[i][0]}).update( this.options[i][1] );
...
_mFormTypes.register('checkbox', Class.create(mFormField, {
  canBeEmpty: true,
  getValueDiv: function(){
    this.inputs = $A();
    var p = new Element('p', {className: 'checkboxes border'});
    for( var i=0; i<this.options.length; i++ ) {
      var label = new Element('label', {className: 'radio'});
      if( this.params && this.params.option_width ) label.setStyle({width: this.params.option_width});
      var input = new Element('input', {name: this.name, type: 'checkbox', value: this.options[i][0]}).observe('change', this.validate.bind(this));
      var span = new Element('span').update( this.options[i][1] );
  ...
  setValue: function(value){
    ...
    for( var i=0; i<this.inputs.length; i++ )
      for( var j=0; j<value.length; j++ )
	      if( this.inputs[i].value==value[j] ) { this.inputs[i].checked = true; }
  ...
  getValue: function(){
    ...
    for( var i=0; i<this.inputs.length; i++ ) {
      if( this.inputs[i].checked )
        result.push( this.inputs[i].value );
    }
    return result;
  },
  _validate: function(){
...
_mFormTypes.register('txt_local_completer', Class.create(_mFormTypes.getClass('varchar'), {
  ...
  onInit: function(params){
    this.options = params.options;
    this.allow_null = Boolean( params.allow_null );
  },
  setValue: function(value){
    this.value = value;
  ...
  _validate: function(){
    ...
    return this.allow_null || Boolean(this.id);
  },
  getValue: function(){
    return this.value;
  }
